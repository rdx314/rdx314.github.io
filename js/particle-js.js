    ...
    function() {
        function t() {
            u = window.innerWidth, c = window.innerHeight, g = {
                x: u / 2,
                y: c / 2
            }, p = document.getElementById("large-header"), p.style.height = c + "px", f = document.getElementById("canvas"), f.width = u, f.height = c, m = f.getContext("2d"), d = [];
            for (var t = 0; u > t; t += u / 30)
                for (var e = 0; c > e; e += c / 60) {
                    var i = t + Math.random() * u / 30,
                        n = e + Math.random() * c / 30,
                        s = {
                            x: i,
                            originX: i,
                            y: n,
            ...
            for (var r in d) {
                var T = new l(d[r], 2 + 2 * Math.random(), "rgba(255,255,255,0.0)");
                d[r].circle = T
            }
        }
        ...
        function h(t) {
            if (t.active)
                for (var e in t.closest) m.beginPath(), m.moveTo(t.x, t.y), m.lineTo(t.closest[e].x, t.closest[e].y), m.strokeStyle = "rgba(247,151,20," + t.active + ")", m.stroke()
        }
        ...
        function l(t, e, i) {
            ...
            ! function() {
                ...
                n.active && (m.beginPath(), m.arc(n.pos.x, n.pos.y, n.radius, 0, 2 * Math.PI, !1), m.fillStyle = "rgba(247,151,20," + n.active + ")", m.fill())
            }
        }
